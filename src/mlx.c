...
/*   Updated: 2022/11/11 14:26:12 by kfergani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(int x, int start, int end, t_window *wind)
{
	...
	while (start <= end)
	{
		my_mlx_pixel_put(wind->image, x, start, 0x00FF0000);
		start++;
	}
}
...
int	raycast(void *globb)
{
	...
	while (x < width)
	{
		...
		line_h = width / perpWallDist;
		start = -line_h / 2 + width / 2;
		if (start < 0)
			start = 0;
		end =  line_h / 2 + width / 2;
		if (end >= width)
			end = width - 1;
